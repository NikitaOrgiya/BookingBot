...
отменённая запись, услуга не помещается, timezone-конвертация), атомарную
...
может) и клиентскую отмену (своя/чужая/поздняя/повторная).
...
отмена своей/чужой/поздней/повторной записи). Без переменной оба
интеграционных теста пропускаются.
...
записи клиенту и `cancellation_notice_minutes`; повторная отмена уже
отменённой записи отклоняется кодом `ALREADY_CANCELLED`.
...
Существующие применённые миграции не переписываются. Миграции применяются
владельцем проекта только после `supabase db diff`/dry-run (в этом
репозитории `supabase db push` не выполнялся — см. финальные замечания
ниже).
