...
- новая таблица `public.appointment_reminders` — журнал напоминаний '24h'/'2h'
  ...
  `notification_deliveries` под ту же задачу, но повторный аудит показал,
  что нельзя доказать её отсутствие на реальном remote/production-проекте
  (см. подробное обоснование в
  `supabase/migrations/20260719100000_appointment_reminders.sql`) — поэтому
  вместо рискованного переименования потенциального production-объекта
  `appointment_reminders` создана как отдельная новая таблица, а
  `notification_deliveries` не тронута ни одной строкой DDL этого этапа;
- триггеры на `appointments` создают до двух напоминаний при подтверждении
  записи (используя уже существующие `business_settings.reminder_first_minutes`/
  `reminder_second_minutes`) и переводят неотправленные напоминания в
  `skipped` при отмене/завершении записи;
- атомарный многострочный claim (`FOR UPDATE SKIP LOCKED` + lease 10 минут)
  через новые `service_role`-only RPC (`claim_due_appointment_reminders`,
  `mark_appointment_reminder_sent/failed/skipped`);
- `lib/reminders/` — переиспользует существующий Telegram-клиент
  (`getBot().api.sendMessage`), retry-политика (до 5 попыток, backoff
  1мин/5мин/15мин/1ч, приоритет `retry_after` Telegram) — в TypeScript,
...
app/                    Next.js App Router: страницы и API-роуты
  ...
  api/cron/reminders/    защищённый endpoint напоминаний (GET-only, Этап 5)
...
lib/
  ...
  reminders/             Этап 5: types.ts, retry.ts (backoff/классификация
                          ошибок Telegram, без БД), message.ts (шаблоны +
                          HTML-экранирование), repository.ts (обёртки RPC),
                          worker.ts (обработка batch'а), cron-handler.ts
                          (тестируемое ядро GET /api/cron/reminders)
...
`CRON_SECRET` (уже существовал в `.env.example` до Этапа 5, ранее нигде не
использовался) теперь защищает `GET /api/cron/reminders`: заголовок
`Authorization: Bearer <CRON_SECRET>` сверяется константным по времени
сравнением; отсутствующий `CRON_SECRET` в окружении — это `401` любому
запросу (fail closed), а не пропуск проверки. Подробности — в разделе
...
`processed_telegram_updates`, `notification_deliveries` (Этап 1, ни разу не
использованная приложением заготовка — оставлена нетронутой, см. "Этап 5"
ниже), `appointment_reminders` (Этап 5, журнал напоминаний).
...
| `notification_deliveries` (Этап 1, не используется) | — | SELECT | SELECT, INSERT, UPDATE |
| `appointment_reminders` (Этап 5) | — | SELECT | SELECT, INSERT, UPDATE |
...
- **Настройки** (`/admin/settings`) — название, IANA timezone (с
  ...
  (на момент Этапа 4 редактировались, но были помечены "Будет
  использоваться после подключения напоминаний" — сами напоминания были
  реализованы позже, на Этапе 5, теми же полями без изменений формы).
...
1. При подтверждении записи (`appointments.status = 'confirmed'`) триггер
   на `appointments` создаёт до двух строк-напоминаний в новой таблице
   `public.appointment_reminders` — по одной на каждый включённый тип
   (`'24h'`/`'2h'`).
2. `GET /api/cron/reminders` (защищённый Bearer-токеном) при каждом
   вызове атомарно захватывает пачку "созревших" напоминаний, отправляет
...
Аудит перед реализацией обнаружил `public.notification_deliveries`
(Этап 1) — таблицу почти с тем же назначением, ни разу не использованную
ни одним RPC, ни одной строкой TypeScript. Первая версия миграции
эволюционировала её через `ALTER TABLE ... RENAME`, чтобы не заводить
дублирующую инфраструктуру. Повторный, более строгий аудит показал, что
этого недостаточно: этот же README утверждает (раздел "Этап 4",
"Результаты проверки"), что миграции Этапа 4 применены к удалённой
(remote) Supabase-базе и совпадают с локальными — а миграции применяются
...
реальном remote/production-проекте. Ни доказать её отсутствие там, ни
проверить remote migration history напрямую в этой среде разработки
невозможно (`supabase migration list`/`db push --dry-run` требуют
слинкованного проекта, которого здесь нет).

Поэтому: `notification_deliveries` **не переименована и не изменена ни
одной строкой DDL** этого этапа — она остаётся ровно такой, какая есть
(если она вообще применена на production, эта миграция её не касается).
Журнал напоминаний живёт в НОВОЙ отдельной таблице
`public.appointment_reminders`, созданной с нуля
(`supabase/migrations/20260719100000_appointment_reminders.sql`). Это
additive, полностью обратно совместимое изменение. Судьба
`notification_deliveries` (оставить навсегда / удалить отдельной
миграцией после явного подтверждения владельца) — вне рамок Этапа 5.
...
| `attempt_count` | число (пере)захватов |
| `next_attempt_at` | когда claim в следующий раз сочтёт строку "due" |
| `locked_at` | момент захвата — основа processing-lease |
| `scheduled_for` | исходно вычисленное время отправки (не меняется retry'ями) |
| `sent_at` | заполнен тогда и только тогда, когда `status = 'sent'` (`CHECK`) |
| `telegram_message_id` | `message_id` успешно отправленного сообщения |
| `last_error_code` / `last_error_message` | безопасный код/текст последней ошибки, без PII, ≤500 символов (`CHECK`) |

`unique (appointment_id, reminder_type)` — одно и то же напоминание не
может существовать дважды для одной записи. RLS/GRANT — те же привилегии,
что и у `notification_deliveries`: `authenticated` — только `SELECT` (через
`is_admin()`-политику), `service_role` — `SELECT`/`INSERT`/`UPDATE`.

### Создание и аннулирование напоминаний — два триггера на `appointments`

- **`create_appointment_reminders_for_confirmed()`** (`AFTER INSERT`,
  `AFTER UPDATE OF status`) — при подтверждении записи читает
  `business_settings.reminder_first_minutes`/`reminder_second_minutes`
  (уже существующие с Этапа 1 настройки, редактируемые на
  `/admin/settings` с Этапа 4 — отдельной настройки под Этап 5 не
  заводится; `NULL` = тип отключён) и создаёт строку-напоминание, только
  если вычисленное время отправки ещё в будущем. Это единое условие
  одновременно реализует оба требования: не создавать просроченные
  напоминания, и не слать 24ч/2ч-напоминание для записи, созданной менее
  чем за 24ч/2ч до начала.
- **`skip_appointment_reminders_on_terminal_status()`** (`AFTER UPDATE OF
  status`) — при отмене/завершении записи (`confirmed` →
  `cancelled`/`completed`/`no_show`) переводит ещё не отправленные
  (`pending`) напоминания этой записи в `skipped`, атомарно, в той же
  транзакции. Строки в `processing` не трогает — эту гонку (запись
  отменили ровно в момент отправки) закрывает пред-отправочная
  перепроверка воркера (см. ниже).
- **Backfill**: та же миграция идемпотентно (`WHERE NOT EXISTS` + `ON
  CONFLICT DO NOTHING`) создаёт напоминания для уже существующих будущих
  подтверждённых записей, созданных до применения миграции — иначе они
  никогда не прошли бы через триггер.

### Атомарный claim и processing-lease

`public.claim_due_appointment_reminders(p_batch_size default 20)` —
единственная точка, откуда напоминания забираются в обработку:
`FOR UPDATE OF ar SKIP LOCKED` над "due" строками (`pending` с
наступившим `next_attempt_at`, либо `processing` с истёкшим
10-минутным lease — зависший воркер не блокирует напоминание навсегда,
тот же принцип, что и `locked_until` в `processed_telegram_updates`,
Этап 3) гарантирует, что два параллельных вызова (два одновременных
срабатывания cron, вручную или из-за особенностей планировщика) никогда
не заберут одну и ту же строку дважды — доказано настоящей параллельной
гонкой двух независимых подключений в
`tests/integration/appointment-reminders.test.ts`, не только
последовательной pgTAP-проверкой.

Три RPC фиксируют исход, каждая — только переход из `processing`:
`mark_appointment_reminder_sent`, `mark_appointment_reminder_failed`
(терминально `→ failed`, либо `→ pending` с вычисленным
`next_attempt_at`), `mark_appointment_reminder_skipped` (пред-отправочная
перепроверка не прошла). Все четыре — `SECURITY INVOKER`, фиксированный
`search_path`, `EXECUTE` только `service_role` (тот же принцип, что и
`booking_functions.sql`: единственный вызывающий уже обходит RLS и имеет
нужные `GRANT`, поднимать привилегии через `DEFINER` незачем).

### Отправка сообщения и повторы (retry)

`lib/reminders/worker.ts` переиспользует существующий Telegram-клиент
(`getBot().api.sendMessage`, тот же бот, что и весь остальной проект) —
отдельной реализации отправки нет. Перед отправкой **обязательна**
пред-отправочная перепроверка актуального статуса записи (запись могла
быть отменена/начаться уже ПОСЛЕ claim, но ДО отправки) —
`mark_appointment_reminder_skipped`, если запись больше не подходит.

Решение "повторить или прекратить, и через сколько" — в TypeScript
(`lib/reminders/retry.ts`, юнит-тестируемо без БД), а не в SQL:

- максимум **5 попыток**; задержки перед попытками 2-5 — **1 минута, 5
  минут, 15 минут, 1 час**;
- Telegram `429 Too Many Requests` — приоритет у `retry_after` из ответа
  API (с потолком 3600 секунд на случай аномального значения) над
  стандартным расписанием;
- Telegram `400`/`403`/`404` (некорректный запрос/бот заблокирован/чат не
  найден) — **не** retryable: терминальная неудача немедленно, без
  ожидания исчерпания всех 5 попыток;
- Telegram `5xx`, сетевые ошибки (`HttpError`), прочие коды — retryable,
  в пределах тех же 5 попыток;
- `last_error_code`/`last_error_message` в БД никогда не содержат сырой
  текст сетевой ошибки: `HttpError` оборачивает ошибку `fetch`, чьё
  сообщение может содержать полный URL запроса (а значит и
  `TELEGRAM_BOT_TOKEN`, часть пути
  `https://api.telegram.org/bot<TOKEN>/sendMessage`) — единственный
  источник текста ошибки — `GrammyError.description` (то, что сам
  Telegram формирует как человекочитаемое объяснение), с обрезкой до 500
  символов.

Ошибка одного чата (например, бот заблокирован конкретным клиентом) не
прерывает обработку остальных напоминаний того же batch'а — ловится
внутри цикла по каждой строке отдельно.

### Идемпотентность и честный крайний случай "ambiguous delivery"

Повторная отправка одного и того же напоминания клиенту исключена в
подавляющем большинстве случаев: `unique(appointment_id, reminder_type)`
не даёт создать одну и ту же строку дважды, claim атомарен, а lease не
даёт двум воркерам взять одну и ту же строку параллельно.

**Честно не решённый (и не решаемый без transactional outbox) крайний
случай:** если процесс воркера обрывается (SIGKILL/serverless
timeout/деплой) уже ПОСЛЕ того, как Telegram реально принял и отправил
сообщение, но ДО того, как успел вызвать
`mark_appointment_reminder_sent`, строка остаётся в `processing` до
истечения lease, после чего может быть перезахвачена и отправлена
повторно — клиент в этом (редком) случае получит напоминание дважды.
Это не баг и не отличается от аналогичного, уже задокументированного
ограничения идемпотентности Telegram-обновлений в Этапе 3 (см. "Этап 3:
Telegram-бот" → "Идемпотентность Telegram-обновлений"): *точно один раз*
доставить сообщение во внешний Telegram API в принципе невозможно без
двухфазного протокола, которого Bot API не предоставляет. Гарантия
распространяется на состояние в БД (ровно один переход в терминальный
статус на попытку), а не на кратность именно исходящего сообщения.

### Шаблоны сообщений

`lib/reminders/message.ts`, `parse_mode: "HTML"` (изолированно от
остального бота, который использует обычный текст — понадобилось именно
здесь для безопасной вставки названия услуги). Название услуги
экранируется (`escapeTelegramHtml`: `&`, `<`, `>`) перед вставкой — и
потому что администратор (источник `service_name_snapshot`) не
доверенный ввод при `parse_mode: "HTML"`, и как единообразная защита от
HTML injection в принципе. Текст для `'2h'` намеренно говорит "**примерно**
через 2 часа" — cron может сработать с небольшой задержкой относительно
точного `scheduled_for`, поэтому сообщение не утверждает "ровно через 2
часа".

### `GET /api/cron/reminders`

Тонкий Route Handler (`app/api/cron/reminders/route.ts`, тот же принцип,
что и `api/telegram/webhook/route.ts`) поверх тестируемого ядра
`lib/reminders/cron-handler.ts`. Экспортирует только `GET` — Next.js сам
вернёт `405` на любой другой метод.

- Авторизация — заголовок `Authorization: Bearer <CRON_SECRET>`,
  константное по времени сравнение (`crypto.timingSafeEqual`), тот же
  принцип, что и у сверки `X-Telegram-Bot-Api-Secret-Token` в Этапе 3.
- **Fail closed**: если `CRON_SECRET` не задан в окружении — `401` любому
  запросу, а не пропуск проверки.
- Ответ — только счётчики (`{ success, claimed, sent, failed, skipped }`)
  и `Cache-Control: no-store`. Ни `chat_id`, ни имя/username клиента, ни
  текст ошибок с чувствительными деталями в ответе или логах никогда не
  фигурируют.

### Выбор планировщика: Supabase Cron, а не Vercel Cron

Нужен вызов endpoint'а примерно раз в 5 минут. Vercel Cron на плане Hobby
ограничен низкой частотой (не чаще раза в день на бесплатном плане;
частые cron — платные планы), а точный план Vercel-проекта нельзя
достоверно определить программно в этой среде разработки (нет доступа к
Vercel API/токену). Supabase Cron (`pg_cron` + `pg_net`, уже расширения
самого проекта) не имеет подобного ограничения по частоте и не требует
дополнительной внешней инфраструктуры. Поэтому выбран **Supabase Cron**
— решение, а не предположение по умолчанию без альтернативы: если в
будущем окажется, что Vercel-план это позволяет и владелец предпочитает
Vercel Cron, конфигурация — это всего один `vercel.json`/cron-запись,
никакой код `lib/reminders/`, `route.ts` менять не придётся.

**Настроен, но не включён.** `supabase/ops/` (вне `supabase/migrations/`
— не применяется автоматически ни `supabase db reset`, ни CI, ни этим PR)
содержит три скрипта, применяемых владельцем вручную через Supabase SQL
Editor:

- `setup-reminder-cron.sql` — создаёт (идемпотентно, по стабильному
  имени `bookingbot-appointment-reminders`) job `cron.schedule(...,
  '*/5 * * * *', ...)`, вызывающий `net.http_get` с URL и
  `Authorization`-заголовком, резолвя оба значения из
  `vault.decrypted_secrets` **в момент каждого выполнения job'а** — не
  как литералы в сохранённом тексте команды (иначе секрет осел бы
  открытым текстом в системной таблице `cron.job`). Требует
  предварительно созданных Vault-секретов `reminders_cron_url` /
  `reminders_cron_secret` (сами значения — только в самом Supabase SQL
  Editor, никогда не в Git/чате/логах).
- `teardown-reminder-cron.sql` — снимает job (секреты в Vault не
  трогает).
- `check-reminder-cron.sql` — только чтение: расписание/статус job'а,
  последние 20 запусков, последние 20 напоминаний со статусом `failed`
  (только `appointment_id`, тип, число попыток, безопасный код/текст
  ошибки — без имени/телефона/email клиента) и общая сводка по статусам.

### Переменные окружения

Ни одной новой переменной не добавлено. `CRON_SECRET` уже существовал в
`.env.example` (был зарезервирован заранее, нигде не использовался) —
Этап 5 впервые начал его читать (`process.env.CRON_SECRET` в
`route.ts`). `TELEGRAM_BOT_TOKEN` не появляется в этой области кода
вообще — весь доступ к Telegram API идёт через уже существующий
`getBot()`.

### Тесты Этапа 5

- **Unit** (`tests/unit/reminders-*.test.ts`, без БД и без сети):
  `reminders-retry` (расписание backoff, приоритет `retry_after`,
  классификация ошибок Telegram, что сетевая ошибка никогда не попадает
  в `last_error_message` дословно), `reminders-message` (шаблоны и
  HTML-экранирование), `reminders-cron-handler` (401 без заголовка/с
  неверным секретом/без `CRON_SECRET` в окружении, безопасная форма
  ответа, отсутствие PII), `reminders-worker` (мок только транспорта —
  claim/mark_*-RPC и `sendMessage`: пустой batch, успешная отправка,
  отмена/начало записи между claim и отправкой → skip, ошибка одного
  чата не блокирует остальные, постоянная `403` → терминальный failed
  сразу, исчерпание 5 попыток → терминальный failed).
- **pgTAP** (`supabase/tests/appointment_reminders.test.sql`): `CHECK`-
  ограничения, `FK`, привилегии всех четырёх RPC на всех трёх ролях,
  триггер создаёт обе строки с корректным `scheduled_for` **и**
  `next_attempt_at` (регрессионный тест — см. ниже, почему это отдельно
  важно), исключение порогов (создание слишком поздно/только для одного
  типа), `NULL`-минуты отключают тип, отмена/завершение → `skipped`
  (кроме `processing`), полный жизненный цикл claim/mark_sent/
  mark_failed/mark_skipped, claim не берёт уже начавшуюся запись.
  `supabase/tests/permissions.test.sql` дополнен (не заменён) новыми
  GRANT/RLS-проверками для `appointment_reminders` — существующие
  проверки `notification_deliveries` остались нетронутыми.
- **Integration** (`tests/integration/appointment-reminders.test.ts`,
  реальный `pg`, `service_role`): создание обоих напоминаний, отмена →
  `skipped`, claim не берёт уже начавшуюся запись, **настоящая
  параллельная гонка** двух независимых подключений за одну и ту же due-
  строку (`total claimed === 1`), повторный claim не берёт `sent`-строку,
  `mark_failed` откладывает следующую попытку, истёкший lease
  перезахватывается.
- **Playwright E2E** — не изменялись: Этап 5 не добавляет UI, поэтому
  добавлять новые сценарии некорректно (нечего проверять в браузере);
  существующий набор Этапа 4 (12 сценариев) остаётся обязательным CI-
  gate'ом без изменений.

**Найденный и исправленный в процессе разработки баг:** `next_attempt_at`
изначально полагался на column default (`now()`), из-за чего
`claim_due_appointment_reminders` (проверяющий именно `next_attempt_at`,
а не `scheduled_for`, для `pending`-строк) считал каждое только что
созданное напоминание немедленно "due", независимо от реального времени
отправки. Обнаружено интеграционным тестом параллельного claim
(неожиданно `total claimed = 2` вместо `1`), исправлено явной установкой
`next_attempt_at = scheduled_for` в триггере и backfill; добавлен
отдельный регрессионный pgTAP-тест.
...
# ожидается: 401, тело без деталей о причине (fail closed)

curl -i -H "Authorization: Bearer <CRON_SECRET из Preview-окружения>" \
     "https://<preview-домен>/api/cron/reminders"
# ожидается: 200, JSON вида {"success":true,"claimed":0,"sent":0,"failed":0,"skipped":0}
# (0 по всем полям, если в Preview-базе нет due-напоминаний — это ожидаемо
# и безопасно: эндпоинт ничего не отправляет, если нечего отправлять)
```

Если владелец хочет увидеть настоящую отправку на Preview: создать (через
тестового Telegram-пользователя, не реального клиента) одну confirmed-
запись с `start_at` на 2-3 минуты позже `reminder_second_minutes`
(например, вручную через SQL Editor Preview-проекта — НЕ через
production-бота), подождать наступления `scheduled_for`, вызвать
эндпоинт тем же `curl` — тестовый Telegram-аккаунт должен получить
...
см. комментарий в файле). Так как отправка полностью идемпотентна на
уровне записи в БД (не UI), откат не требует ни миграции назад, ни
удаления данных — просто останавливает будущие вызовы эндпоинта.

### Мониторинг без PII

`supabase/ops/check-reminder-cron.sql` — единственный рекомендованный
способ посмотреть на неудачные напоминания: выбирает только
`appointment_id`, тип, статус, число попыток и безопасный код/текст
последней ошибки (уже обрезанный до 500 символов и никогда не содержащий
токен/URL/сырой ответ Telegram) — ни имя, ни телефон, ни email, ни
`chat_id` клиента ни в одном из четырёх запросов файла не выбираются.
